...
// Define the type for alertDialogConfig separately
  ...
  ({ onThoughtRecalled, onEmptyRecalledThoughts, isExternallyHeaderDictating, onStopHeaderDictationParent }, ref) => {
    ...
    const actuallyProcessInputText = useCallback(async (textToProcess: string) => {
      ...
      try {
            ...
            for (const actionItem of nonSearchActionItems) { 
              ...
              if (itemToAdd && targetListKey && targetListName) {
                setAlertDialogConfig({
                  ...
                  description: (
                    <>
                      The AI refined this to: <strong>{actionItem.title}</strong>. Add{' '}
                      <strong>{itemToAdd}</strong> to your {targetListName}?
                    </>
                  ),
        ...
        console.error("Error during text processing or AI interaction:", error);
        toast({ title: "Error Processing Thought", description: (error instanceof Error ? error.message : "An unknown error occurred."), variant: "destructive" });
        actionExecutedWithoutDialog = true; 
      } finally {
    ...
    const handleProcessInputText = useCallback(() => {
      ...
      if (isDashboardDictationActive) {
        ...
        // onend of dashboardDictationRecognitionRef will call actuallyProcessInputText
      } else if (isHeaderDictationActiveInternal) {
        // If header dictation was active, process its final transcript
        // This assumes stopHeaderDictation or its onend already populated inputText correctly
            ...
            if (dashboardDictationPauseTimeoutRef.current) {
              ...
              for (let i = 0; i < event.results.length; ++i) {
                ...
                if (event.results[i].isFinal && i < event.results.length -1) {
                    currentSessionTranscript += ' '; // Add space between final segments
                }
              }
              setInputText(currentSessionTranscript.trim()); // Live update in textarea
          
              const lowerTranscript = currentSessionTranscript.toLowerCase();
              const endCommand = WAKE_WORDS.END_DICTATION.toLowerCase();
              const stopCommand = WAKE_WORDS.STOP_DICTATION.toLowerCase();
          
              if (lowerTranscript.includes(endCommand) || lowerTranscript.includes(stopCommand)) {
                let finalText = currentSessionTranscript.trim();
                if (lowerTranscript.includes(endCommand)) {
                    finalText = finalText.substring(0, finalText.toLowerCase().lastIndexOf(endCommand)).trim();
                } else if (lowerTranscript.includes(stopCommand)) {
                    finalText = finalText.substring(0, finalText.toLowerCase().lastIndexOf(stopCommand)).trim();
                }
                ...
                // Reset the timeout for automatic stop if speech continues
